...
import PredictionUserDataModel, {
    Position,
    Status
} from "../../models/PredictionDataModel";
...
export const usePredictionViewModel = () => {
    ...
    const contract = Prediction__factory.connect(
        PREDICTION_ADDRESSES[
            process.env
    ...
    const predict = useCallback(
        (
            ...
            predictUsecase({
                ...
                contract,
                available,
                currentRound: rounds[currentRound].epoch,
                callbacks
            }),
    ...
    const getRounds = useCallback(
        () => getRoundsAction(contract, address, active)(dispatch),
        [dispatch, contract, address, active]
    );
    const getUserRound = useCallback(
        (round) =>
            getUserRoundAction(contract, round, address, active)(dispatch),
    ...
    const initPrediction = useCallback(
        () => initPredictionAction(contract, address, active)(dispatch),
        [dispatch, contract, address, active]
    );

    let nos = Object.keys(rounds);
    let userPredictionData: (PredictionUserDataModel | boolean)[] = nos.map(
        (no) => {
            const round = rounds[no];
            if (!round.user) return false;

            const myPrediction = round.user.position;
            const coinPredicted = getPredictionTokenName(
                round.user?.token
            ) as keyof typeof TokenImages;
            ...
            const bears = toNumberLib(round.bears[index])
                .div(round.bets[index].toString())
                .times(100)
                .toFixed(0, 4);
            const bulls = toNumberLib(round.bulls[index])
                .div(round.bets[index].toString())
                .times(100)
                .toFixed(0, 5);
            ...
            let status;
            if (
                ((lockedPrice.gt(closingPrice) &&
                    myPrediction === Position.BEAR) ||
                    (closingPrice.gt(lockedPrice) &&
            ...
            return {
                myPrediction,
                coinPredicted,
                coinPredictedIcon: coinPredicted
                    ? TokenImages[coinPredicted]
                    : "",
    ...
    let _userPredictionData = userPredictionData?.filter(
        (data) => data !== false
    ) as unknown as PredictionUserDataModel[];
    _userPredictionData.sort((a, b) => {
        return +a.round > +b.round ? -1 : 1;
    });

    const poolsEntered = _userPredictionData.length;
    const roundsWon = _userPredictionData.filter(
        (round) => round.status === Status.WON
    ).length;
    const roundsLost = _userPredictionData.filter(
        (round) => round.status === Status.LOST
    ).length;
    const unsuccessful = _userPredictionData.filter(
        (round) => round.status === Status.UNSUCCESSFUL
    ).length;
    const unsuccessfulTokens = _userPredictionData
        .filter(
            (round) => round.status === Status.UNSUCCESSFUL && !round.claimed
        )
        .reduce((oldRound, newRound) => {
            return (
                rounds[newRound.round].user?.amount.add(oldRound) || oldRound
            );
        }, BigNumber.from(0));
    const _unsuccessfulTokens = displayTokenValue(
        unsuccessfulTokens.toString(),
        18,
        5
    );
    const overview = {
        ...
        roundsWon,
        unsuccessful,
        unsuccessfulTokens: _unsuccessfulTokens
    };
    const unsuccessfulRounds = _userPredictionData
        .filter(
            (round) => round.status === Status.UNSUCCESSFUL && !round.claimed
        )
